...
export const Sidebar: React.FC<{ isOpen?: boolean; onClose?: () => void }> = ({ isOpen = false, onClose }) => {
  const { currentPath, navigate, userRole, currentUser, biasFlags, compareCandidateIds, logout, fairScreeningMode } = useApp();
  ...
  return (
    <aside
      className={`fixed inset-y-0 left-0 z-40 w-64 bg-slate-900 text-slate-300 border-r border-slate-800 flex flex-col transition-transform duration-200 lg:translate-x-0 ${
    ...
    >
      ...
      <div className="px-4 py-3 mx-3 mt-3 rounded-lg bg-slate-800/80 border border-slate-700/60 flex items-center justify-between">
        ...
        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
          fairScreeningMode ? 'bg-emerald-500/20 text-emerald-300' : 'bg-slate-700 text-slate-400'
        }`}>
                ...
                {item.badge && (
                  <span
                    className={`px-1.5 py-0.5 text-[10px] font-bold rounded-full border ${
                      item.badgeColor || (isActive ? 'bg-indigo-700 text-white border-indigo-500' : 'bg-slate-800 text-slate-300 border-slate-700')
                    }`}
                  >
                    {item.badge}
                  </span>
